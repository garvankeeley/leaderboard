...
_week_classes_cache = {}
def get_reportweek_table(week_num):
...
def get_reportweek_table_for_contributor_and_tile(contributor, tile):
    week_class = get_reportweek_table(current_week_number())
...
def insert_or_update_reportweek(contributor, tile):
    ...
    with session.begin(subtransactions=True):
        ...
        existing = get_reportweek_table_for_contributor_and_tile(contributor, tile)
        if existing:
            return existing
